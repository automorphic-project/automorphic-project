...
In this section, the underlying field is of characteristic zero.
...
\begin{enumerate}
 ...
 \item If $\alpha\in\Phi$, let $\mathfrak t_\alpha \in \mathfrak h$ be the image of $\alpha$ under the isomorphism: $\mathfrak h^*\to\mathfrak h$ defined by a non-degenerate invariant symmetric form $(\,,\,)$ on $\mathfrak g$. Then, for all $X\in \mathfrak g_\alpha, Y\in\mathfrak g_{-\alpha}$ we have: $[X,Y] = (X,Y) h_\alpha$. 
 ...
 \item Take $(\,,\,)$ to be the Killing form, and use it to identify $\mathfrak h = \mathfrak h^*$. Let $E$ be the real vector space spanned by the roots. Then, the Killing form is positive definite on $E$, and for any $\alpha\in \Phi$ the orthogonal reflection $w_\alpha$ on $\mathfrak h^*$ across the hyperplane perpendicular to $\alpha$ fixes $\Phi$.
\end{enumerate}
\end{theorem}
...
\begin{enumerate}
 ...
 \item Start by choosing elements $x=e \in \mathfrak g_\alpha$, $y=f\in\mathfrak g_{-\alpha}$ with $(e,f) = \frac{2}{(t_\alpha,t_\alpha)}$, and set  $h_\alpha = \frac{2}{(t_\alpha,t_\alpha)} t_\alpha$. Then, $(h,e,f)$ is an $\mathfrak{sl}_2$-triple, and we denote its span by $\mathfrak{sl}_{2,\alpha}$. The subspace
 $$ \mathfrak m = \mathfrak h_\alpha \oplus \bigoplus_{n\in \mathbb Z, n \ne 0} \mathfrak g_{n\alpha}$$ decomposes, as an $\mathfrak{sl}_{2,\alpha}$-module, into finite-dimensional irreducible representations with even weights. Since the zero weight space is one-dimensional, this implies (from the classification of irreducible, finite-dimensional representations of $\mathfrak{sl}_2$) that it is irreducible. It contains the adjoint representation of $\mathfrak{sl}_2$, therefore $\mathfrak m = \mathfrak{sl}_{2,\alpha}$. 
 \item The previous point proves that there are no integral multiples of $\alpha$ in $\Phi$, other than $\pm \alpha$. Similarly, again by the classification of irreducible, finite-dimensional representations of $\mathfrak{sl}_2$, the only other multiples are half-integral, and if one of them appears, then $\frac{\alpha}{2}$ must appear. But, repeating this argument with $\frac{\alpha}{2}$ in place of $\alpha$, we get that $2\frac{\alpha}{2}$ cannot appear, a contradiction. Hence, $\frac{\alpha}{2}\ne \Phi$.
 \item If we had taken $(\,,\,)$ to be the Killing form, then $(t_\lambda,t_\lambda) = \text{tr}((\text{ad}(t_\lambda))^2) = \sum_{\alpha\in\Phi} \alpha(t_\lambda)^2 >0$. The spaces $\mathfrak h_\alpha$ span $\mathfrak h$, hence the Killing form is positive definite.
 
 Fix now a positive definite form, and for any $\beta\in \Phi$ consider the subspace
 $$\mathfrak n = \mathfrak g_{\beta + \mathbb Z \alpha}.$$
 It is a $\mathfrak{sl}_{2,\alpha}$-stable subspace with weights of the same parity, and each weight space is one-dimensional, therefore it is an irreducible representation of $\mathfrak{sl}_{2,\alpha}$, and must include a range of weights differing by $2$; therefore, 
 $$\Phi \cap (\beta + \mathbb Z \alpha) = \left\{\beta+ n\alpha\right\}_{n=-r}^q$$
 for some nonnegative integers $r, q$. This has $r+q+1$ weights, therefore the lowest weight must be $-(r+q)$, the heighest weight must be $(r+q)$, and $\left< \beta, h_\alpha\right> = r-q$. The orthogonal reflection of $\beta$ is, therefore, 
 $$ w_\alpha(\beta) = \beta - 2\frac{(\beta,\alpha)}{(\alpha,\alpha)} \alpha = \beta - \left< \beta, h_\alpha\right> \alpha = \beta - (r-q)\alpha \in \Phi.$$
\end{enumerate}
\end{proof}

\begin{remark}
 \label{remark-indep-innerprod}
Any two invariant symmetric forms are multiples of each other on every simple summand of $\mathfrak g$, and different simple summands are orthogonal, so the reflection $w_\alpha$ does not depend on the choice of inner product. 
\end{remark}
...
\end{itemize}
and there exists an inner product on $E$ such that 
\begin{enumerate}
 \item $W$ is generated by the reflections $w_\alpha$ perpendicular to the elements $\alpha \in \Phi$;
 \item for all $\beta\in \Phi$, $w_\alpha \beta\in \beta + \mathbb Z \alpha$. 
\end{enumerate}
...
In most references, the inner product on $E$ is part of the data of a root system. However, it is better not to fix an inner product unless needed.
\end{remark}
...
Notice that $\mathfrak h_\alpha$ does not depend on the choice of $e$ and $f$. We will denote the set of coroots by $\check\Phi$. The simple reflection $w_\alpha$ on $\mathfrak h^*$ associated to the root $\alpha$ (see Theorem \ref{theorem-root-system} and Remark \ref{remark-indep-innerprod}) can be written:
\begin{equation}
