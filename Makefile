...
LIJST = introduction representationtheory representations-compact \
    ...
    algebraicgroups reductiveforms galoiscohomology \
    representations-local plancherel discreteseries \
    automorphicspace automorphicforms
...
ifndef INSTALLDIR
	...
	tar -c -f $(INSTALLDIR)/automorphic-pdfs.tar --exclude book.pdf --transform=s@tags/tmp@automorphic-pdfs@ tags/tmp/*.pdf
	git archive --format=tar HEAD | (cd $(INSTALLDIR) && tar xf -)
	cp automorphic-project.tar.bz2 $(INSTALLDIR)
...
#	rm -f automorphic-project.tar.bz2
...
.PHONY: backup
backup:
	git archive --prefix=automorphic-project/ HEAD | bzip2 > \
		../automorphic-project_backup.tar.bz2

.PHONY: tarball
tarball:
	git archive --prefix=automorphic-project/ HEAD | bzip2 > automorphic-project.tar.bz2
